...
func (h *PodHandler) DownloadXshellFile(c *gin.Context) {
	// 使用查询参数获取 namespace 和 name
	namespace := c.Query("namespace")
	name := c.Query("name")
	if namespace == "" || name == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "缺少 namespace 或 name 参数"})
		return
	}
	...
	nodeIP := h.getNodeIP(ctx, pod.Spec.NodeName)
	if nodeIP == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "无法获取节点 IP"})
		return
	}
	...
	if sshPort == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "无法获取 SSH 端口"})
		return
	}
	...
	// 生成 Xshell 会话文件内容
	// Xshell 使用 INI 格式的 .xsh 文件
	xshContent := fmt.Sprintf(`[SESSION]
Host=%s
Port=%d
UserName=root
Password=%s
Protocol=SSH
Version=5.0
`, nodeIP, sshPort, password)

	// 设置响应头，触发浏览器下载
	filename := fmt.Sprintf("%s.xsh", name)
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=%s", filename))
	c.Header("Content-Type", "application/octet-stream")
	c.String(http.StatusOK, xshContent)
}
