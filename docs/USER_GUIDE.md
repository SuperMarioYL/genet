...
│                    Genet 平台                           │
├─────────────────────────────────────────────────────────┤
│  ✓ 自助式 GPU Pod 创建        ✓ SSH / VSCode / Web Shell │
│  ✓ 持久化存储自动挂载         ✓ 镜像保存与复用          │
│  ✓ GPU 利用率实时监控         ✓ 资源配额管理            │
...
1. Pod 进入 `Running` 后，卡片和详情页会出现 **「Web Shell」** 按钮
2. 点击后会在新标签页打开浏览器终端
3. 页面会自动创建一个临时 shell 会话，并连接到 Pod 主容器的 `/bin/sh`
4. 关闭页面后会话立即结束，不保留历史记录
